#!/usr/bin/env python3
"""Mirror public report JSON files to a public Gitee repository."""
...
USER_AGENT = "weibo-miniapp-gitee-sync/1.0"
...
def validate_report(file_name: str, value: Any) -> dict[str, Any]:
    ...
    if file_name == "personal.json":
        expected = ("recommendations", "items", "monitored_posts")
    else:
        expected = ("items", "top10", "rss_candidates")
...
def decode_remote_content(remote: dict[str, Any] | None) -> bytes | None:
    if not remote or not remote.get("content"):
        return None
    encoded = str(remote["content"]).replace("\n", "")
...
def main() -> int:
    ...
    if not source_base_url.startswith("https://"):
        raise RuntimeError("SOURCE_BASE_URL must use HTTPS")

    for file_name in REPORT_FILES:
        content = download_report(source_base_url, file_name)
...
if __name__ == "__main__":
    ...
    except Exception as error:  # Keep Actions output concise and token-free.
